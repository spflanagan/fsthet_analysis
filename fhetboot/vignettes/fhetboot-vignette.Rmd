...
**fhetboot** also contains the option for calculating p-values. To calculate p-values, run `p.boot` on the bootstrapping output. For this function to work, the output of `fst.boot` must be in the format it's in when you've run multiple bootstrapping replicates. It takes the data.frame of actual Fst values and either the bootstrapping output or the output from running `fst.boot.means`. You can then use a false discovery rate or p-value correction to coorect for multiple testing.

```{r}
#run bootstrapping 1 time using replicate to get it in the right format
...
In some cases you may prefer to calculate confidence intervals and p-values from the empirical dataset iteslf, without performing bootstrapping. For these cases, run `fst.boot` with `bootstrap = FALSE`.
```{r}
